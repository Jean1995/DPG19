...
\begin{frame}{Introduction}
      \begin{itemize}
        ...
        \item Possibility to use \textbf{different parametrizations} $\rightarrow$ Study \textbf{systematic uncertanties}
...
\begin{tikzpicture}[node distance = 1.7cm, auto]
 ...
 $- \left\langle \frac{\mathrm{d}E}{\mathrm{d}x} \right\rangle \propto E \int_{v_\text{min}}^{v_\text{cut}} v \frac{\mathrm{d}\sigma}{\mathrm{d}v} \mathrm{d}v$};
    \node [block, below of=two] (three) {Calculate type of stochastic loss};
    \node [block, below of=three] (four) {Sample the stochastic energy loss via:\\\vspace{5px} $\frac{1}{\sigma} \int_{v_\text{cut}}^{v\left(\xi\right)} \frac{\mathrm{d}\sigma}{\mathrm{d}v} \mathrm{d}v = \xi$};
    \node [decision, right of=three, node distance=7.5cm] (repeat) {Repeat until $E < e_0$ or\\ particle reached section border};

    % Draw edges
    \path [line] (one) -- (two);
    \path [line] (two) -- (three);
    \path [line] (three) -- (four);
    \path [line, dashed] (four) -| (repeat);
